...
def _run_inputs(run_id: str) -> dict:
...
def start_srt(run_id: str, body: dict) -> dict:
    ...
    try:
        job = srt_runner.start(run_id, paths.run_dir(run_id), inputs["audio"], model, device, mode, max_chars)
...
def srt_job(job_id: str) -> dict:
    status = srt_runner.job_status(job_id)
    if status is None:
...
def srt_job_log(job_id: str, offset: int = Query(0, ge=0), limit: int = Query(200, ge=1, le=2000)) -> dict:
    return srt_runner.get_log(job_id, offset, limit)


@router.post("/jobs/{job_id}/cancel")
def srt_job_cancel(job_id: str) -> dict:
    return {"cancelled": srt_runner.cancel(job_id), "job_id": job_id}
