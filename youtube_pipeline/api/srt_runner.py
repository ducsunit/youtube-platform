"""Detached runner for Japanese script-to-SRT jobs, isolated per channel namespace."""
...
class SrtRunner:
    """At most one SRT job per user/channel namespace; legacy scope remains supported."""
    ...
    def jobs_dir(user_id: str | None = None, channel_id: str | None = None) -> Path:
        if bool(user_id) != bool(channel_id):
            raise ValueError("user_id và channel_id phải được truyền cùng nhau")
        return (
            paths.channel_root(user_id, channel_id) / "runtime" / "srt-jobs"
            if user_id and channel_id
            else paths.srt_jobs_dir()
        )
    ...
    def _active_unlocked(
        ...
        jobs_dir = jobs_dir or paths.srt_jobs_dir()
        ...
        *,
        ...
        with self._lock:
            ...
            (jobs_dir / (job_id + ".pid")).write_text(
                json.dumps({"pid": proc.pid, "kind": "srt", "run_id": run_id, "user_id": user_id, "channel_id": channel_id}),
                encoding="utf-8",
            )
            job = {
                "id": job_id,
                ...
                "output_path": "subtitles/subtitles.srt",
            }
    ...
    def _waiter(self, proc: subprocess.Popen, job: dict, jobs_dir: Path, scope: tuple[str | None, str | None]) -> None:
        ...
        with self._lock:
            ...
            self._finished[(scope, job_id)] = finished
    ...
    def job_status(self, job_id: str, *, user_id: str | None = None, channel_id: str | None = None) -> Optional[dict]:
        ...
        jobs_dir = self.jobs_dir(user_id, channel_id)
        log_path = jobs_dir / (job_id + ".log")
        info = _read_pid_info(jobs_dir / (job_id + ".pid"))
        ...
        if log_path.is_file():
            ...
            return {"id": job_id, "user_id": user_id, "channel_id": channel_id, "status": "complete" if code == 0 else "failed", "exit_code": code, "output_path": "subtitles/subtitles.srt"}
        return None
